...
export class Dragger extends React.Component {
  ...
  onResizeStart = (event) => {
    ...
    let moveDirect = 'se'
    if (className.indexOf('s-resize') >= 0) {
      moveDirect = 's'
    } else if (className.indexOf('w-resize') >= 0) {
      moveDirect = 'w'
    }
    this.setState({
      originX: originX,
  ...
  onResizing = (event) => {
    ...
    */
    ...
    const { moveDirect } = this.state
    if (moveDirect === 's') {
      delete st.w
    } else if (moveDirect === 'w') {
      delete st.h
    }
    this.setState(st, () => {
